...
module.exports = {
    ...
    callback: async (message, args) => {
        ...
        if (oppenent === '@everyone' || oppenent === '@here') return message.channel.send('Nice try.')
        if (!oppenent) return message.channel.send("Please mention someone you would like to play against.");
        if (oppenent.id === challenger.id) return message.channel.send('You can\'t play against yourself.')
        if (oppenent.user.bot === true) return message.channel.send('You can\'t play against a bot.')
